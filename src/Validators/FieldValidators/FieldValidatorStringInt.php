    ...
    public function validate( mixed $value, array $flags = [] ):bool {
        if ( gettype( $value ) !== 'string' && gettype( $value ) !== 'integer' ) {
            throw new FieldValidatorException(
                sprintf( 'Field %s is expected to be a string or an integer, but is %s', $this->key, gettype( $value ) )
            );
        }

        if ( ! ( (int)$value ) ) {
            throw new FieldValidatorException(
                sprintf( 'Field %s is expected to be a string or an integer, but is falsey when cast', $this->key )
            );
        }
